...
namespace ETSL.InstrDriver.Base
{
    public class InstrDriverException : Exception
    {
        ...
        protected InstrDriverException( System.Runtime.Serialization.SerializationInfo info, 
    ...
    public class VisaInstrDriver: INotifyPropertyChanged
    {
        ...
        protected bool Open()
        {
            try
            {
                ...
                if (VisaAddress != string.Empty)
                {
                    this.mbSession = (MessageBasedSession)ResourceManager.GetLocalManager().Open(VisaAddress, AccessModes.NoLock, 10000);
                    this.mbSession.SetAttributeBoolean(AttributeType.SuppresEndEn, false);
                    this.mbSession.SetAttributeBoolean(AttributeType.TermcharEn, false);
                    //this.mbSession.SetAttributeInt16(AttributeType.Termchar, 10);
                    this.mbSession.SetAttributeBoolean(AttributeType.SendEndEn, true);
                }
                else
                {
                    throw new InstrDriverException("VISA address is empty!");
        ...
        public void SendCommand(string command)
        {
            try
            {
                ...
                {
                    throw new InstrDriverException("Instrument has not been initialized yet!");
                }
            }
            catch (Exception ex)
            {
